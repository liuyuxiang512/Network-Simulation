...
int max_q = 0;
int current_max_q = 0;
ofstream maxqueue_file("maxqueue.dat");
ofstream delay_file("delay.dat");
ofstream lossrate_file("lossrate.dat");
ofstream throughput_file("throughput.dat");
...
void getInfo()
{
	...
	//cout << "  Max Queue at ";
	maxqueue_file << Simulator::Now().GetSeconds() << "\t" << current_max_q << endl;
...
int main(int argc, char *argv[])
{
	...
	for (int t = 0; t < 1200; t++) {
		...
		// Mean Delay: 
		delay_file << StartTime + 0.1*t << '\t';
		delay_file << delay / n_couple[t] << endl;

		// Mean Throughput: 
		throughput_file << StartTime + 0.1*t << '\t';
		throughput_file << throughput / n_couple[t] << endl;

		// Mean Loss Rate: 
		lossrate_file << StartTime + 0.1*t << '\t';
		lossrate_file << loss / n_couple[t] << endl;
